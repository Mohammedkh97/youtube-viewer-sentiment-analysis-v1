...
class ModelFactory:
    ...
    _MANUAL_PARAMS = {
        ...
        "xgboost": dict(n_estimators=300, learning_rate=0.1, max_depth=6, subsample=0.8, colsample_bytree=0.8),
    ...
    def get(
        ...
        if X_train is None or y_train is None:
            raise ValueError(f"X_train and y_train must be provided for search_strategy='{search_strategy}'")
        ...
        if search_strategy == "grid":
            ...
            gs = GridSearchCV(base, grid, cv=cv, scoring=scoring, n_jobs=-1, verbose=verbose)
        ...
        if search_strategy == "random":
            base = cls._build_base(name_lower, random_state)
            grid = param_grids.get(name_lower, {})
            rs = RandomizedSearchCV(base, grid, n_iter=n_iter, cv=cv, scoring=scoring, random_state=random_state, n_jobs=-1, verbose=verbose)
